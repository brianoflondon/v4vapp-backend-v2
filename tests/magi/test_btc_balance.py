...
from v4vapp_backend_v2.magi.btc_balance import MagiBTCBalance, get_btc_balance_by_account
...
async def test_get_btc_balance_by_account_success(mocker):
    ...
    result = await get_btc_balance_by_account("hive:devser.v4vapp")
...
async def test_get_btc_balance_by_account_empty_result(mocker):
    ...
    result = await get_btc_balance_by_account("hive:missing")
...
async def test_get_btc_balance_by_account_graphql_error(mocker):
    ...
    with pytest.raises(RuntimeError, match="GraphQL errors"):
        await get_btc_balance_by_account("hive:devser.v4vapp")
...
async def test_get_btc_balance_by_account_fallback_to_second_endpoint(mocker):
    ...
    result = await get_btc_balance_by_account("devser.v4vapp")
