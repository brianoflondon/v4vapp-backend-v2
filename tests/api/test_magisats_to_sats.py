...
from v4vapp_backend_v2.helpers.service_fees import calculate_fee_estimate_msats, calculate_fee_msats
...
def test_magisats_to_sats_basic(client):
    ...
    magisats = 10_000
    r = client.get("/v2/crypto/magisats_to_sats/", params={"magisats": magisats})
    assert r.status_code == 200
    data = r.json()

    assert data["original_sats"] == magisats
...
def test_magisats_to_sats_larger_amount_has_higher_fee(client):
    ...
    r_small = client.get("/v2/crypto/magisats_to_sats/", params={"magisats": 10_000})
    r_large = client.get("/v2/crypto/magisats_to_sats/", params={"magisats": 1_000_000})
    ...
    assert r_large.json()["forwarding_fee_estimate_msats"] > r_small.json()["forwarding_fee_estimate_msats"]
