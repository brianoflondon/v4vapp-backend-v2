...
def human_readable_datetime(dt_obj: datetime) -> str:
    """
...
class MyJSONFormatter(logging.Formatter):
    ...
    def _prepare_log_dict(self, record: logging.LogRecord):
        created_datetime = dt.datetime.fromtimestamp(record.created, tz=dt.timezone.utc)
        timestamp = created_datetime.isoformat()
        created_datetime_str = human_readable_datetime(created_datetime)

        # Ensure datetime and level are the first fields
        always_fields = OrderedDict(
            {
                "level": record.levelname,
                "datetime": created_datetime_str,
        ...
        # Add remaining attributes from the log record
        remaining_fields = OrderedDict(
            {
                key: val
        ...
        message: OrderedDict[str, Any] = OrderedDict()
        message.update(always_fields)
        message.update(remaining_fields)
