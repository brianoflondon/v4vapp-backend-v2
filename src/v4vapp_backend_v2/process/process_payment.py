...
async def record_payment(payment: Payment, quote: QuoteResponse) -> list[LedgerEntry]:
    ...
    # TODO: #249 Implement the system for expense accounts
    # An expense payment should credit an Expense account (non contra) instead of an Asset Contra account
    ...
    keysend_message = getattr(payment.custom_records, "keysend_message", None)
    if payment.invoice_description == "Not set":
        user_memo = keysend_message or ""
    else:
        user_memo = f"Send {payment.value_sat_rounded} sats to Node {payment.destination} (fee: {payment.fee_sat_rounded}) {keysend_message or ''}".strip()
    ...
    outgoing_ledger_entry = LedgerEntry(
        ...
        debit=LiabilityAccount(
            name="VSC Liability",
            sub=debit_cust_id,  # This is the CUSTOMER or server if via server
        ),
