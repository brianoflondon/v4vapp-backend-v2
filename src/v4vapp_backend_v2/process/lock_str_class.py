...
class LockStr(AccName):
    ...
    Redis lock value note:
    The Redis key value for each lock (for example a 32-char hex string) is an
    ownership token generated by redis-py. It identifies the lock owner instance
    so only that owner can safely release or extend the lock.
    """
    ...
    async def acquire_lock(
        ...
        Notes:
            redis.asyncio.lock.Lock generates and stores an owner token in Redis
            as the lock value. The token is used by Redis lock scripts to verify
            ownership during release/extend operations.

            Difference between timeout and blocking_timeout in acquire_lock:

            timeout:
            This is lock lifetime in Redis (TTL), passed into RedisLock at lock_str_class.py:204.
            If timeout is 30, the lock key auto-expires after ~30s.
            If timeout is None, lock has no expiry.

            blocking_timeout:
            This is how long the caller waits to acquire the lock before failing, passed at lock_str_class.py:207.
            Your wrapper also enforces this with elapsed-time checks and raises CustIDLockException at lock_str_class.py:247.
    ...
    async def locked(
        ...
        Usage:
            async with LockStr("object_id").locked(
                request_details="processing object_id"
            ):
                # critical section with lock held
                ...
        ...
        Raises:
            CustIDLockException: If the lock could not be acquired within the blocking timeout
