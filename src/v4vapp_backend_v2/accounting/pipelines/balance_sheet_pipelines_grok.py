                        ...
                        "$add": [
                            ...
                            },
                            {
                                "$cond": [
                                    ...
                                    {"$multiply": ["$conv_signed.credit.msats", -1]},
                                    0,
                                ]
                            },
                        ]
                        ...
                        "$add": [
                            {
                                "$cond": [
                                    ...
                                    {"$multiply": ["$conv_signed.debit.msats", -1]},
                                    0,
                                ]
                            },
                            {
    ...
    pipeline: Sequence[Mapping[str, Any]] = [
                ...
                "debit_sums": [
                    {
                        "$group": {
                            ...
                            "count": {"$sum": 1},
                            "debit_hbd": {"$sum": "$conv_signed.debit.hbd"},
                            "debit_hive": {"$sum": "$conv_signed.debit.hive"},
                            "debit_msats": {"$sum": "$conv_signed.debit.msats"},
                            "debit_sats": {"$sum": "$conv_signed.debit.sats"},
                            "debit_usd": {"$sum": "$conv_signed.debit.usd"},
                        }
                    }
                ],
                "credit_sums": [
                    {
                        "$group": {
                            ...
                            "count": {"$sum": 1},
                            "credit_hbd": {"$sum": "$conv_signed.credit.hbd"},
                            "credit_hive": {"$sum": "$conv_signed.credit.hive"},
                            "credit_msats": {"$sum": "$conv_signed.credit.msats"},
                            "credit_sats": {"$sum": "$conv_signed.credit.sats"},
                            "credit_usd": {"$sum": "$conv_signed.credit.usd"},
                        }
                    }
                ],
            }
        ...
        {"$unwind": "$all"},
        {
            "$group": {
                "_id": "$all._id",
                "count": {"$sum": "$all.count"},
                "debit_hbd": {"$sum": "$all.debit_hbd"},
                "debit_hive": {"$sum": "$all.debit_hive"},
                "debit_msats": {"$sum": "$all.debit_msats"},
                "debit_sats": {"$sum": "$all.debit_sats"},
                "debit_usd": {"$sum": "$all.debit_usd"},
                "credit_hbd": {"$sum": "$all.credit_hbd"},
                "credit_hive": {"$sum": "$all.credit_hive"},
                "credit_msats": {"$sum": "$all.credit_msats"},
                "credit_sats": {"$sum": "$all.credit_sats"},
                "credit_usd": {"$sum": "$all.credit_usd"},
            }
        },
        {
            "$project": {
                ...
                "hbd": {
                    "$cond": [
                        {"$eq": ["$_id.type", "Asset"]},
                        {"$subtract": ["$debit_hbd", "$credit_hbd"]},
                        {"$subtract": ["$credit_hbd", "$debit_hbd"]},
                    ]
                },
                "hive": {
                    "$cond": [
                        {"$eq": ["$_id.type", "Asset"]},
                        {"$subtract": ["$debit_hive", "$credit_hive"]},
                        {"$subtract": ["$credit_hive", "$debit_hive"]},
                    ]
                },
                "msats": {
                    "$cond": [
                        {"$eq": ["$_id.type", "Asset"]},
                        {"$subtract": ["$debit_msats", "$credit_msats"]},
                        {"$subtract": ["$credit_msats", "$debit_msats"]},
                    ]
                },
                "sats": {
                    "$cond": [
                        {"$eq": ["$_id.type", "Asset"]},
                        {"$subtract": ["$debit_sats", "$credit_sats"]},
                        {"$subtract": ["$credit_sats", "$debit_sats"]},
                    ]
                },
                "usd": {
                    "$cond": [
                        {"$eq": ["$_id.type", "Asset"]},
                        {"$subtract": ["$debit_usd", "$credit_usd"]},
                        {"$subtract": ["$credit_usd", "$debit_usd"]},
                    ]
                },
            }
        },
