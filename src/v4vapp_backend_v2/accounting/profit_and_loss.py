...
async def generate_profit_and_loss_report(
    ...
    """
    Generates a Profit and Loss report summarizing Revenue and Expense accounts.

    Args:
        df (pd.DataFrame, optional): DataFrame of ledger entries. If empty, fetches from database.
        start_date (datetime, optional): Start date for the report period. Defaults to earliest entry.
        end_date (datetime, optional): End date for the report period. Defaults to current UTC time.
        collection_name (str, optional): Database collection name. Defaults to "ledger".

    Returns:
        dict: A dictionary with:
            - Revenue: {account_name: {sub: {sats, hive, hbd, usd}}}
            - Expenses: {account_name: {sub: {sats, hive, hbd, usd}}}
            - Net Income: {sats, hive, hbd, usd}
    """
    ...
    if df.empty:
        return {
            ...
            "Net Income": {"sats": 0.0, "hive": 0.0, "hbd": 0.0, "usd": 0.0},
        }
    ...
    for _, row in pl_df.iterrows():
        ...
        if sub not in pl_report[category][name]:
            pl_report[category][name][sub] = {"sats": 0.0, "hive": 0.0, "hbd": 0.0, "usd": 0.0}

        if unit.lower() == "sats":
            pl_report[category][name][sub]["sats"] += amount
        ...
        elif unit.lower() == "msats":
            pl_report[category][name][sub]["sats"] += amount / 1000
            pl_report[category][name][sub]["usd"] += usd
            pl_report[category][name][sub]["hive"] += (
                usd / spot_rates["hive_to_usd"] if spot_rates["hive_to_usd"] != 0 else 0.0
            )
    ...
    for category in ["Revenue", "Expenses"]:
        for account_name in pl_report[category]:
            ...
            total_sats = sum(sub_acc["sats"] for sub_acc in sub_accounts.values())
            total_usd = sum(sub_acc["usd"] for sub_acc in sub_accounts.values())
            total_hive = sum(sub_acc["hive"] for sub_acc in sub_accounts.values())
            total_hbd = sum(sub_acc["hbd"] for sub_acc in sub_accounts.values())
            pl_report[category][account_name]["Total"] = {
                "sats": round(total_sats, 3),
    ...
    total_revenue = sum(
        acc["Total"]["usd"] for acc in pl_report["Revenue"].values() if "Total" in acc
    )
    total_expenses = sum(
        acc["Total"]["usd"] for acc in pl_report["Expenses"].values() if "Total" in acc
    )
    net_income_usd = total_revenue - total_expenses
    pl_report["Net Income"] = {
        "sats": round(net_income_usd / spot_rates["sats_to_usd"], 0)
        if spot_rates["sats_to_usd"] != 0
        else 0.0,
        "hive": round(net_income_usd / spot_rates["hive_to_usd"], 3)
        if spot_rates["hive_to_usd"] != 0
        else 0.0,
        "hbd": round(net_income_usd / spot_rates["hbd_to_usd"], 3)
        if spot_rates["hbd_to_usd"] != 0
        else 0.0,
        "usd": round(net_income_usd, 2),
    }
...
def profit_and_loss_printout(
    ...
    """
    Formats a Profit and Loss report into a readable string, displaying SATS, HIVE, HBD, and USD.
    ...
    for account_name, sub_accounts in pl_report["Revenue"].items():
        for sub, balance in sub_accounts.items():
            if sub == "Total":
                continue
            output.append(
                f"{account_name:<40} {sub:<17} {balance['sats']:>10,.0f} {balance['hive']:>12,.3f} {balance['hbd']:>12,.3f} {balance['usd']:>12,.2f}"
            )
        total = sub_accounts.get("Total", {})
        output.append(
            f"{'Total ' + account_name:<40} {'':<17} {total.get('sats', 0):>10,.0f} {total.get('hive', 0):>12,.3f} {total.get('hbd', 0):>12,.3f} {total.get('usd', 0):>12,.2f}"
        )
    output.append("-" * max_width)
    ...
    # Net Income
    net_income = pl_report["Net Income"]
    output.append(
        f"{'Net Income':<40} {'':<17} {net_income['sats']:>10,.0f} {net_income['hive']:>12,.3f} {net_income['hbd']:>12,.3f} {net_income['usd']:>12,.2f}"
    )
    output.append("=" * max_width)
