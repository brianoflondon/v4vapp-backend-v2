...
def one_account_transactions_pipeline(
    ...
    """
    SIMPLE PIPELINE FOR one_account_balance ONLY.

    Returns raw sorted transactions (exactly the same document shape the old
    pipeline produced before running totals). Running totals are now computed
    in Python (much more reliable, easier to debug, no more $setWindowFields
    or O(N²) map/filter issues).

    This eliminates the MongoDB-side complexity that was causing timeouts
    and parse errors on large accounts.
    """
    filter = filter or {}
    if account:
        debit_match_query: dict[str, Any] = {
